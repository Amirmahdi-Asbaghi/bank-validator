...
```bash
curl -X POST \
  -F "file=@data/samples/valid_small.csv" \
...
Use this to explore the API without reading this doc.
...
| `bankval_runs_total` | counter | `path` = `sync` \| `async_kafka` \| `async_celery` | Validation runs by execution path |
